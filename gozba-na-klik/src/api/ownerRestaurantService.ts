...
// Dobavljanje porudzbina na cekanju za vlasnika restorana AZ
export async function getOwnerPendingOrders(ownerId: number) {
  const res = await api.get(`/owners/${ownerId}/orders/pending`);
  return res.data;
}
